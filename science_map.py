...
def main(path):
    ...
    # single phrases annotations
    nlp_annotated_annotations = [[[nlp_annotator.annotate(span) for span in spans] for spans in span_set] for span_set
                                 in
                                 span_sets]
    # filter them if they contain the tagged spans on all sides of the set
    nlp_annotated_annotations = [sps for sps in nlp_annotated_annotations if sps and len(sps) > 1
                                 and all(
                                         all(
                                             any(s['kind'] == kind for s in sp)
                                             for kind in annotation_kind_set)
                                     for sp in sps)]
    # uniqueness based on tagged spans (looks complicated because of the unique-making based on a dict)
    nlp_annotated_annotations = list(
        {
            ''.join(f['text']
                    for g in e for f in g): e
        for e in nlp_annotated_annotations}.values())
    ...
    def opposed_relations(annotation_sets, connect_on='CONTRAST'):
        to_connect = [annotation for annotation_set in annotation_sets for side in annotation_set for annotation in side
                      if annotation['kind'] == connect_on]
    ...
    out_csv = path + "/relations.csv"
    logging.info(f'writing output file to {out_csv}')
...
if __name__ == '__main__':
    parser = ArgumentParser(description='finding relations of differences')
