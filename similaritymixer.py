...
class SimilarityMixer:
    def __init__ (self, similarity_composition, n=1, verbose=False):
        r''' Setup, which functions are taken to compute a similarity matrix to choose from some natural language expressions, that are already analysed in the manner of :mod:`~predicatrix.Predication`.
            ...
            two vectors and a vector valued function,
               ...
               .. math:: \overrightarrow{make}(x_{1,n}, x_{2,m}) = \langle f_1(x_{1,n}, x_{2,m}), f_2(x_{1,n}, x_{2,m}), \dots f_k(x_{1,n}, x_{2,m}) \rangle
            ...
            .. math::
                ...
                \overrightarrow{w} \bullet \sum D\overrightarrow{make}(\overrightarrow{x_1}, \overrightarrow{x_2})  \Delta n = \overrightarrow{w} \bullet
                \begin{bmatrix}
    ...
    def choose(self, data,
        ...
        lf = len(self.funs)
        if 0 and exs1 == exs2:
            inds    = np.array(list(itertools.combinations(range(l1), r=2)))
            combos = list(zip(itertools.combinations(exs1, r=2), list(itertools.combinations(range(l1), r=2))))
        else:
            inds   = np.array(list(itertools.product(range(l1),range(l2))))
            combos = list(zip(itertools.product(exs1, exs2), list(itertools.product(range(l1),range(l2)))))

        res_cube = np.zeros(shape=(lf, l1, l2))

        for axis, fun in enumerate(self.funs):
            with timeit_context(""):
                res, beams = list(zip(*map(lambda t: fun(t[0][0], t[0][1], i1=t[1][0], i2=t[1][1]), combos)))
                res_cube[axis][inds[:,0], inds[:,1]] = res
                for d in beams:
                    self.beam.update(d)
        ...
        if self.verbose:
            ...
            table = list(zip(["scaling thresholds", "comparison values in matrix", "choosing thresholds", 'choosing comparison value'],
                             [self.minmax_defaults[1], res_cube.max(axis=0), self.thresholds_max, trans_cube.max(axis=0)],
                             [self.minmax_defaults[0], res_cube.min(axis=0), self.thresholds_min, trans_cube.min(axis=0)]))
        ...
        if layout=='hdbscan':
            ...
            if all(cluster_labels == -1):
                 logging.info('Switching to Sprectral Clustering because of not enough instances')
        ...
        if layout == "1:1":
            ...
            ''' compute coordinates within in the cube by divmod, that means, try to divide through the length of the rows (=y)
    ...
    def expressions_list (self, left_value, right_values, exs1, exs2):
        ...
        try:
            triggers = [[self.beam[l][r]  if l in self.beam and r in self.beam[l] else
                         self.beam[r][l] # in case of symmetrical 1:1, they may be turned around, but there is only one possibility
                        for r in right_values[l_i]]
                        for l_i, l in enumerate(left_value)]
        except KeyError:
            logging.error ('beam has wrong ids %s ' % self.funs)
    ...
    def multi_kind_tup_sim(fun, n=2, assign = None, layout=None):
        '''
        ...
        >>> SimilarityMixe            sim = 0
r.multi_cross2tup_sim(SimilarityMixer.same_expression_sim, n=2)(*data2)
        ...
        def broadcast(exs1, exs2):
            if not assign:
                sides = list(zip(exs1, exs2))
                flat_sorted_sides = [sorted([e for ex in side for e in ex],  key=lambda x: x['kind']) for side in sides]
                grouped_sides     = [(kind, list(group)) for side in flat_sorted_sides for kind, group in itertools.groupby(side, key=lambda d: d['kind'])]
            else:
                flat_sorted_sides1 = sorted([ex for side in exs1 for ex in side],  key=lambda x: x['kind'])
                grouped_sides1     = {kind: list(group) for kind, group in itertools.groupby(flat_sorted_sides1, key=lambda d: d['kind'])}
                flat_sorted_sides2 = sorted([ex for side in exs2 for ex in side],  key=lambda x: x['kind'])
                grouped_sides2     = {kind: list(group) for kind, group in itertools.groupby(flat_sorted_sides2, key=lambda d: d['kind'])}
                if layout=='both':
                    grouped_sides      = [(grouped_sides1[kind1] + grouped_sides1[kind1],
                                           grouped_sides2[kind2] + grouped_sides2[kind2][::-1])
                                          for kind1, kind2 in assign.items()]
                else:
                    grouped_sides      = [(grouped_sides1[kind1], grouped_sides2[kind2]) for kind1, kind2 in assign.items()]

                grouped_sides      = [(i, l) for i, l in enumerate(list(zip(*[ g for tup in grouped_sides for g in tup])))]
        ...
        def multi_kind_tup_sim_generated (exs1,exs2, i1=0, i2=0):
            if not exs1 or not exs2:
                raise ValueError ("one of the expressions is empty")

            # broadcast to other one, succession matters
            # if one contains less, zip takes the shortest

            values, dicts= zip(*[broadcast(exs1, e2) for e2 in itertools.permutations(exs2)])

            x, i = max(values), min(values)
            d = (x-i)
            if x == i:
                d = 1

            i, res = max(enumerate(values), key=lambda x: x[1])
            res =  res * d
            b = {i1: {i2: dicts[i]}}
            if d<0.05:
                res = 0
            return res, b
        return multi_kind_tup_sim_generated
    ...
    def multi_cross2tup_sim(fun, n=2):
        ...
        def multi_cross2tup_sim (exs1,exs2):
            ...
            try:
                ...
                logging.error('Some value to compare has not a enough or too many values to be broadcasted')
    ...
    def boolean_sim (attrib_dict):
        ''' According to a dictionary, that says, which attribute in the one of the pair must also in the attribute
            ...
            ...       'lemma': ['all', 'your', 'life'],
            ...       'dep_' : ['det', 'poss', 'subj']}
            >>> make = SimilarityMixer.boolean_sim (d)
            >>> make (p1,p2)
            (2, {})
        ...
        if isinstance(exs1[0], dict) and isinstance(exs2[0], dict):
            ...
            for trigger in t_s:
                try:
                    ...
                    raise TypeError(
                        "one of the complex didn't have the list-tuple-dict-list-specification\n"
                        "types: \n%s,\n%s,\n%s,\n%s" %
